...
class LeagueTable
	...
	def add_match(match)
		...
		if !teams[name1]
			teams[name1] = Team.new name1
		end

		if !teams[name2]
			teams[name2] = Team.new name2
		end
	...
	def get_goals_for(team_name) # Returns the no. of goals a team has scored, 0 by default
		if teams[team_name]
	...
	def get_goals_against(team_name) # Returns the no. of goals a team has conceeded (had scored against them), 0 by default
		if teams[team_name]
